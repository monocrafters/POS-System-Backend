...
import { adminNavItems, type AdminNavItem, type AdminSectionId, } from "./admin-nav";
...
export function AdminSidebar({ active, onNavigate }: AdminSidebarProps) {
    ...
    const byGroup = groups.map((group) => ({
        group,
        items: adminNavItems.filter((i) => i.group === group),
    }));
    const displayName = user?.fullName ?? "Administrator";
    const username = user?.username ?? "admin";
    return (<aside className="flex h-full w-[260px] shrink-0 flex-col border-r border-neutral-200/80 bg-white">
      <div className="border-b border-neutral-100 px-4 py-5">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <span className="mt-1.5 inline-flex items-center gap-1 rounded-md bg-red-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#E31837] ring-1 ring-red-100">
      ...
      <nav className="scrollbar-modern flex-1 overflow-y-auto px-3 py-4">
        {byGroup.map(({ group, items }) => items.length > 0 ? (<div key={group} className="mb-5">
